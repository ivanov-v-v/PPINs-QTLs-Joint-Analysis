...
identify_QTLs <- function(genotypes_filename, expression_filename, output_filename, P_THRESHOLD = 0.05) {
  library(MatrixEQTL)
...
identify_QTLs(
'eQTLs/genotypes_2011 (copy).csv',
'eQTLs/expression_2011.csv',
'eQTLs/qtls_2011.csv',
 0.05
)
...
QTLs_df = na.omit(read.table("~/Science/eQTL_analysis/data/eQTLs/qtls_2011.csv", sep = '\t', stringsAsFactors = FALSE, header = TRUE))
...
write.table(QTLs_df[QTLs_df$q_value <= 0.05,], "~/Science/eQTL_analysis/data/eQTLs/qtls_2011.csv", sep = '\t', row.names = FALSE)
