...
def basic_module_info(interactome_graph, modules_dict, modules_type, make_plots=True, simplify=False):
    ...
    return pd.DataFrame(
        [(mname, len(modules_dict[mname]), mgraph.vcount(), mgraph.ecount())
         for mname, mgraph in module_graphs_dict.items()],
        columns=["module_name", "genes_count", "genes_with_interactions", "interactions_count"]
    )
